...
export const onlinePresense = supabase.channel("onlineUsers", {
...
export const useStore = (props) => {
  ...
  const [directMessage, setDMessages] = useState([]);
  ...
  useEffect(() => {
    ...
    const messageListener = supabase
      ...
      .subscribe();
    // Listen for new and deleted DM
    const directMessageListener = supabase
      .channel("direct_messages")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "direct_messages" },
        (payload) => setDMessages((messages) => messages.concat(payload.new))
      )
      .on(
        "postgres_changes",
        ...
        (payload) =>
          setDMessages((messages) =>
            messages.filter((message) => message.id !== payload.old.id)
          )
      )
      .subscribe();
  ...
  return {
    ...
    directMsg: directMessage.map((x) => ({
      ...x,
      author: users.get(x.reciever_id),
    })),
    channels:
      channels !== null
 ...
 * @param {number} sender_id The author
 */
export const addDM = async (message, reciever_id, sender_id) => {
  try {
    let { data } = await supabase
      .from("direct_messages")
      .insert([{ message, reciever_id, sender_id }])
      .select();
    return data;
  } catch (error) {
    console.log("error", error);
...
/**
 * Delete a DM from the DB
 * @param {number} message_id
 */
export const deleteDm = async (message_id) => {
  try {
    let { data } = await supabase
      .from("direct_messages")
      .delete()
